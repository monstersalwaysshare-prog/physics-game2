...
function App()
{
  ...
  */
  ...
  // ✅ Limit how many cards this level uses based on targetCards
  let levelCardLimit = totalCards;
  if (targetCards === "10")
  {
    levelCardLimit = Math.min(10, totalCards);
  }
  else if (targetCards === "20")
  {
    levelCardLimit = Math.min(20, totalCards);
  }
  // if "full", leave levelCardLimit = totalCards
 ...
 if (!started)
{
  ...
  function handleAnswer(answerIsYes)
  {
    ...
    const outOfCards = (nextIndex >= levelCardLimit); // ⭐ attempt to fix 10 cards still 42 issue
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* ⭐ change totalCards to levelCardLimit to fix issue 10 cards game not 42*/}
      {!studentMode && (
        <p style={{ marginTop: 0, marginBottom: isMobile ? 6 : 10 }}>
          This level has {levelCardLimit} cards. You may make up to 3 mistakes before the level ends.
        </p>
      )}

      <ProgressBar
        current={cardIndex + 1}
        total={levelCardLimit}
        isMobile={isMobile}
      />

      {!studentMode && (
        <h3 style={{ marginTop: 0, marginBottom: isMobile ? 6 : 10 }}>
          Card {cardIndex + 1} of {levelCardLimit}
        </h3>
      )}
      ...
      {showDebugUI && (
        <div style={{ display: "flex", gap: 12, marginTop: isMobile ? 10 : 16 }}>
          ...
          <button onClick={goNext} disabled={cardIndex === levelCardLimit - 1}>Next</button>
        </div>
      )}
    </div>
  );
